...
// 日志缓冲区尺寸配置（8MB 大缓冲区）
static constexpr int FixedLargeBuffferSize = 8 * 1024 * 1024;

// 日志系统相关参数
static constexpr double BufferWriteTimeout = 3.0;                 // 超时刷盘时间（秒）
static constexpr int64_t FileMaximumSize = 1024LL * 1024 * 1024;  // 单文件最大日志体积，1GB
...
public:
    ...
    explicit AsyncLogging(const char* filepath = nullptr);
